  ...
  (
    ...
    return (
      <ProgressPrimitive.Root
        ref={ref}
        className={cn(progressVariants({ color, size }), "relative", className)}
        {...props}>
        <ProgressPrimitive.Indicator
          className={cn(
            "flex-1 transition-all h-full w-full flex items-center justify-center relative ",
            className,
            {
        ...
        {showValue && (
          <span className=" text-right pr-1 text-[10px] text-black block w-full absolute right-0 -top-1">
            {value}%
          </span>
        )}
      </ProgressPrimitive.Root>
