...
import EvaluatedViewModal from "@/components/evaluated/view-evaluated-modal";
...
const getFinishedIndicatorData = (value: string): ProcessIndicatorItem => ({
  id: 2,
  name: "Finalizados",
  count: value,
  icon: (
...
const getInProcessIndicatorData = (value: string): ProcessIndicatorItem => ({
  id: 3,
  name: "En proceso",
  count: value,
  icon: (
...
const getSentIndicatorData = (value: string): ProcessIndicatorItem => ({
  id: 5,
  name: "Enviados",
  count: value,
  icon: (
...
export default function ProcessesPage() {
  ...
  const [evaluatedSelected, setEvaluatedSelected] = useState<Evaluated>();
  ...
  const getEvaluateds: (queryKey: QueryKey) => Promise<{
    rows: Evaluated[];
    totalPages: number;
    currentPage: number;
  ...
  const getStatuses = async () => {
    api.get("/evaluateds/filters/status").then((response) => {
      setStatuses(response.data.data);
    });
  };

  const onLoad = () => {
    Promise.all([getHierarchyList(), getStatuses()]);
    getIndicators();
  };
  ...
  const onFilterChange = (filters: ColumnFiltersState) => {
    ...
    const hierachicalFilter = filters.find(
      (filter: ColumnFilter) => filter.id === "hierachical_level"
    );
    ...
    setFilters((prev) => ({
      ...
      hierarchical_level_id: (hierachicalFilter?.value as number) || null,
  ...
  const downloadUrl = useMemo(() => {
    ...
    return `${apiUrl}/evaluateds/search/export?${params.toString()}&access_token=${accessToken}`;
  ...
  const onView = (item: Evaluated) => {
    setOpenViewModal(true);
    setEvaluatedSelected(item);
  };

  const getIndicators = async () => {
    api.get("/evaluateds/indicators/get").then((response) => {
      const data = response.data.data;
      setIndicators({
        total: data.total,
        sent: data.sent,
  ...
  return (
    <Fragment>
      ...
      <div className="flex items-center justify-between mb-10">
        <h2 className="text-3xl font-medium text-dark-blue">Evaluados</h2>
      </div>
      <div className="h-6">
        {isFetching ? (
          <Progress
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 md:col-span-12 mb-4">
          <div className="grid grid-cols-1 sm:grid-cols-4 2xl:grid-cols-4 gap-4">
            <ProcessIndicator
              indicator={getTotalIndicatorData(indicators.total)}
              iconBgClasses="bg-dark-blue"
            />
            <ProcessIndicator
              indicator={getSentIndicatorData(indicators.sent)}
              iconBgClasses="bg-primary"
            />
            <ProcessIndicator
              indicator={getInProcessIndicatorData(indicators.in_process)}
              iconBgClasses="bg-destructive"
            />
            <ProcessIndicator
              indicator={getFinishedIndicatorData(indicators.finished)}
      ...
      <Card className="rounded-t-2xl p-6">
        <CardContent className="p-0">
          <DataTable
            ...
            data={data?.rows || []}
            columns={getColumns({ statuses, onView })}
